...
def process_cache(cache_dir: Path, output_root: Path, thresholds: argparse.Namespace) -> Tuple[int, int]:
    ...
    for selection in iterate_cluster_selections(cache_dir, cache_key, report_path, thresholds):
        cluster_dir = output_root / cache_key / f"cluster-{selection.cluster_id}"
        copied = 0
        ...
        if copied:
            ...
            total_clusters += 1
            total_files += copied
            print(f"[INFO] Copied {copied} samples for cache {cache_key} cluster {selection.cluster_id} -> {cluster_dir}")
